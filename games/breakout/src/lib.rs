...
//! clock (https://daybrite.dev/docs/canvas): an eased, finger-tracking paddle that can smash the
//! ball up or down, multi-hit armored bricks, five power-ups with timer badges, combos with score
//! popups, particle bursts, a ball trail and a side-wall predictor, level-clear and game-over
//! cards, a pause menu, settings, and a how-to-play sheet. A composite Day piece: pure composition
//! over `day_pieces` plus the shared `gamekit` chrome, so it renders the same on every backend.
